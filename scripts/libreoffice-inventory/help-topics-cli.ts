/**
 * @fileoverview Implements the explicit write-command that validates the pinned baseline and generates canonical LibreOffice XHP topic metadata.
 */
...
import { createHelpTopicInventory } from "./help-topics";
import { validateBaseline } from "./inventory";
import { parseBaselineManifest } from "./manifest";

/** Defines explicit paths accepted by the XHP help-topic inventory command. */
export interface HelpTopicCliOptions {
  /** UTF-8 baseline manifest path. */
  readonly baselinePath: string;
  /** Explicit generated JSON destination path. */
  readonly outputPath: string;
  /** Ignored core checkout root whose helpcontent2 submodule is inspected read-only. */
  readonly referenceRoot: string;
}

/**
 * Parses strict XHP help-topic inventory command options.
 *
 ...
 */
export function parseHelpTopicCliOptions(argumentsList: readonly string[]): HelpTopicCliOptions {
  const values = new Map<string, string>();
  for (let index = 0; index < argumentsList.length; index += 1) {
    const option = argumentsList[index];
    const value = argumentsList[index + 1];
    if (option !== "--baseline" && option !== "--reference-root" && option !== "--output") {
      throw new Error(`Unsupported help topic inventory option: ${option}`);
    }
  ...
  if (baselinePath === undefined || outputPath === undefined || referenceRoot === undefined) {
    throw new Error(
      "Usage: inventory:help-topics -- --baseline <path> --reference-root <path> --output <path>",
    );
  }
...
/**
 * Validates the full baseline and writes canonical XHP topic metadata to the caller-selected output path.
 *
 ...
 * @returns A promise resolving after canonical topic inventory output is written.
 */
export async function runHelpTopicCli(
  argumentsList: readonly string[],
  readTextFile: (filePath: string) => Promise<string>,
  writeTextFile: (filePath: string, contents: string) => Promise<void>,
  git: GitExecutor = createGitExecutor(),
): Promise<void> {
  const options = parseHelpTopicCliOptions(argumentsList);
  ...
  const helpCorpus = manifest.corpora.find(isHelpCorpus) as (typeof manifest.corpora)[number];
  const helpRoot = path.join(
    options.referenceRoot,
    path.relative(manifest.referencePath, helpCorpus.referencePath),
  );
  const paths = (await git.run(helpRoot, ["ls-files", "-z"])).split("\0").filter(isNonEmptyPath);
  const inventory = createHelpTopicInventory(helpCorpus.commit, paths);
  await writeTextFile(options.outputPath, `${JSON.stringify(inventory, null, 2)}\n`);
}

/**
 * Identifies the pinned help corpus declaration from the validated manifest.
 *
 * @param corpus - One validated corpus declaration.
 * @param corpus.id - Stable identifier of the candidate corpus.
 * @returns True only for the helpcontent2 corpus.
 */
function isHelpCorpus(corpus: { readonly id: string }): boolean {
  return corpus.id === "helpcontent2";
}
...
if (isDirectModule(import.meta.url, process.argv[1])) {
  await runHelpTopicCli(process.argv.slice(2), readUtf8File, writeUtf8File);
}
