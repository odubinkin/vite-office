/**
 * @fileoverview Implements the explicit write-command that validates the pinned baseline and generates canonical LibreOffice dictionary-file metadata.
 */
...
import { createDictionaryFileInventory } from "./dictionaries";

/** Defines explicit paths accepted by the dictionary-file inventory command. */
export interface DictionaryFileCliOptions {
  /** UTF-8 baseline manifest path. */
  readonly baselinePath: string;
  /** Explicit generated JSON destination path. */
  readonly outputPath: string;
  /** Ignored core checkout root whose dictionaries submodule is inspected read-only. */
  readonly referenceRoot: string;
}

/**
 * Parses strict dictionary-file inventory command options.
 *
 ...
 */
export function parseDictionaryFileCliOptions(
  argumentsList: readonly string[],
): DictionaryFileCliOptions {
  const values = new Map<string, string>();
  for (let index = 0; index < argumentsList.length; index += 1) {
    const option = argumentsList[index];
    const value = argumentsList[index + 1];
    if (option !== "--baseline" && option !== "--reference-root" && option !== "--output") {
      throw new Error(`Unsupported dictionary file inventory option: ${option}`);
    }
  ...
  if (baselinePath === undefined || outputPath === undefined || referenceRoot === undefined) {
    throw new Error(
      "Usage: inventory:dictionaries -- --baseline <path> --reference-root <path> --output <path>",
    );
  }
...
/**
 * Validates the full baseline and writes canonical dictionary-file metadata to the caller-selected output path.
 *
 ...
 * @returns A promise resolving after canonical dictionary inventory output is written.
 */
export async function runDictionaryFileCli(
  argumentsList: readonly string[],
  readTextFile: (filePath: string) => Promise<string>,
  writeTextFile: (filePath: string, contents: string) => Promise<void>,
  git: GitExecutor = createGitExecutor(),
): Promise<void> {
  const options = parseDictionaryFileCliOptions(argumentsList);
  ...
  const dictionariesCorpus = manifest.corpora.find(
    isDictionariesCorpus,
  ) as (typeof manifest.corpora)[number];
  const dictionariesRoot = path.join(
    options.referenceRoot,
    path.relative(manifest.referencePath, dictionariesCorpus.referencePath),
  );
  const paths = (await git.run(dictionariesRoot, ["ls-files", "-z"]))
    .split("\0")
    .filter(isNonEmptyPath);
  const inventory = createDictionaryFileInventory(dictionariesCorpus.commit, paths);
  await writeTextFile(options.outputPath, `${JSON.stringify(inventory, null, 2)}\n`);
}

/**
 * Identifies the pinned dictionaries corpus declaration from the validated manifest.
 *
 * @param corpus - One validated corpus declaration.
 * @param corpus.id - Stable identifier of the candidate corpus.
 * @returns True only for the dictionaries corpus.
 */
function isDictionariesCorpus(corpus: { readonly id: string }): boolean {
  return corpus.id === "dictionaries";
}
...
if (isDirectModule(import.meta.url, process.argv[1])) {
  await runDictionaryFileCli(process.argv.slice(2), readUtf8File, writeUtf8File);
}
