/**
 * @fileoverview Implements the explicit write-command that validates the pinned core checkout and generates canonical UITest Python source-target provenance records.
 */
...
/** Defines explicit paths accepted by the UITest Python source-target inventory command. */
export interface UITestSourceTargetCliOptions {
...
/**
 * Parses strict UITest Python source-target inventory command options.
 *
 ...
 */
export function parseUITestSourceTargetCliOptions(
  argumentsList: readonly string[],
): UITestSourceTargetCliOptions {
  ...
  for (let index = 0; index < argumentsList.length; index += 1) {
    ...
    if (
      ...
      throw new Error(`Unsupported UITest source-target inventory option: ${option}`);
    }
  ...
  if (
    ...
    throw new Error(
      "Usage: inventory:ui-test-source-targets -- --baseline <path> --reference-root <path> --constructors <path> --output <path>",
    );
  }
...
/**
 * Validates the full baseline, links UITest module roots to constructor IDs, and writes canonical Python source-target JSON.
 *
 ...
 * @returns A promise resolving after canonical UITest Python source-target inventory output is written.
 */
export async function runUITestSourceTargetCli(
  argumentsList: readonly string[],
  readTextFile: (filePath: string) => Promise<string>,
  writeTextFile: (filePath: string, contents: string) => Promise<void>,
  git: GitExecutor = createGitExecutor(),
): Promise<void> {
  const options = parseUITestSourceTargetCliOptions(argumentsList);
  ...
  const declarations = await readUITestModuleRootDeclarations(
    options.referenceRoot,
    trackedPaths.filter(isUITestMakefilePath),
    readTextFile,
  );
...
/**
 * Reads and parses all UITest makefiles that may hold Python module-root declarations.
 *
 * @param referenceRoot - Validated local core checkout root.
 * @param makefilePaths - Git-tracked UITest makefile paths to inspect.
 * @param readTextFile - UTF-8 file reader boundary.
 * @returns Parsed UITest module-root declarations carrying exact makefile provenance.
 */
async function readUITestModuleRootDeclarations(
  referenceRoot: string,
  makefilePaths: readonly string[],
  readTextFile: (filePath: string) => Promise<string>,
): Promise<readonly UITestModuleRootDeclaration[]> {
  const declarationsByMakefile = await Promise.all(
    makefilePaths.map(
      /**
       * Reads one pinned makefile and attaches its relative path to every parsed module root.
       *
      ...
      async function readMakefile(
        ...
        return extractUITestModuleRootTargets(sourceText).map(
          /**
           * Attaches immutable makefile provenance to one parsed UITest module root.
           *
           * @param target - Parsed module root without source text retention.
           * @returns One declaration record ready for constructor linkage.
           */
          function attachDeclarationPath(target): UITestModuleRootDeclaration {
...
/**
 * Determines whether a tracked path may contain UITest module-root declarations.
 *
 * @param pathValue - Exact Git-tracked core path to inspect.
 * @returns True only for UITest-named makefiles.
 */
function isUITestMakefilePath(pathValue: string): boolean {
  return pathValue.includes("UITest") && pathValue.endsWith(".mk");
}
...
if (isDirectModule(import.meta.url, process.argv[1])) {
  await runUITestSourceTargetCli(process.argv.slice(2), readUtf8File, writeUtf8File);
}
