/**
 * @fileoverview Implements the explicit write-command that validates the pinned core checkout and generates canonical PythonTest module provenance records.
 */
...
/** Defines explicit paths accepted by the PythonTest module inventory command. */
export interface PythonTestModuleCliOptions {
...
/**
 * Parses strict PythonTest module inventory command options.
 *
 ...
 */
export function parsePythonTestModuleCliOptions(
  argumentsList: readonly string[],
): PythonTestModuleCliOptions {
  ...
  for (let index = 0; index < argumentsList.length; index += 1) {
    ...
    if (
      ...
      throw new Error(`Unsupported PythonTest module inventory option: ${option}`);
    }
  ...
  if (
    ...
    throw new Error(
      "Usage: inventory:python-test-modules -- --baseline <path> --reference-root <path> --constructors <path> --output <path>",
    );
  }
...
/**
 * Validates the full baseline, links PythonTest module declarations to constructor IDs, and writes canonical Python module JSON.
 *
 ...
 * @returns A promise resolving after canonical Python module inventory output is written.
 */
export async function runPythonTestModuleCli(
  argumentsList: readonly string[],
  readTextFile: (filePath: string) => Promise<string>,
  writeTextFile: (filePath: string, contents: string) => Promise<void>,
  git: GitExecutor = createGitExecutor(),
): Promise<void> {
  const options = parsePythonTestModuleCliOptions(argumentsList);
  ...
  const declarations = await readPythonTestModuleDeclarations(
    options.referenceRoot,
    trackedPaths.filter(isPythonTestMakefilePath),
    readTextFile,
  );
...
/**
 * Reads and parses all PythonTest makefiles that may hold Python module declarations.
 *
 * @param referenceRoot - Validated local core checkout root.
 * @param makefilePaths - Git-tracked PythonTest makefile paths to inspect.
 * @param readTextFile - UTF-8 file reader boundary.
 * @returns Parsed Python module declarations carrying exact makefile provenance.
 */
async function readPythonTestModuleDeclarations(
  referenceRoot: string,
  makefilePaths: readonly string[],
  readTextFile: (filePath: string) => Promise<string>,
): Promise<readonly PythonTestModuleDeclaration[]> {
  const declarationsByMakefile = await Promise.all(
    makefilePaths.map(
      /**
       * Reads one pinned makefile and attaches its relative path to every parsed Python module.
       *
      ...
      async function readMakefile(
        ...
        return extractPythonTestModuleTargets(sourceText).map(
          /**
           * Attaches immutable makefile provenance to one parsed Python module.
           *
           * @param target - Parsed Python module without source text retention.
           * @returns One declaration record ready for constructor linkage.
           */
          function attachDeclarationPath(target): PythonTestModuleDeclaration {
...
/**
 * Determines whether a tracked path may contain PythonTest module declarations.
 *
 * @param pathValue - Exact Git-tracked core path to inspect.
 * @returns True only for PythonTest-named makefiles.
 */
function isPythonTestMakefilePath(pathValue: string): boolean {
  return pathValue.includes("PythonTest") && pathValue.endsWith(".mk");
}
...
if (isDirectModule(import.meta.url, process.argv[1])) {
  await runPythonTestModuleCli(process.argv.slice(2), readUtf8File, writeUtf8File);
}
