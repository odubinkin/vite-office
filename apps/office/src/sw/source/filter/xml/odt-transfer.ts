/** @fileoverview Owns the ODT worker's structured-clone graph independently from durable storage. */
...
import { decodeSfxItemSet, encodeSfxItemSet } from "../basflt/item-codec";

/** Primitive graph record for one numbering level. */
interface OdtNumberFormatRecord {
...
interface OdtNumberRuleRecord {
  readonly automatic: boolean;
  readonly formats: readonly OdtNumberFormatRecord[];
  readonly listId: string;
  readonly name: string;
}

/** Primitive graph record for one paragraph-style collection. */
interface OdtStyleRecord {
...
export interface OdtWriterTransferGraph {
  readonly numRules: readonly OdtNumberRuleRecord[];
  readonly odtTransferModelVersion: 1;
  readonly textFormatCollections: readonly OdtStyleRecord[];
  readonly textNodes: readonly OdtTextNodeRecord[];
}

/** Encodes the model at the browser boundary. @param document - Canonical graph. @returns Current record. */
export function encodeOdtWriterTransferGraph(document: SwDoc): OdtWriterTransferGraph {
  return {
...
export function decodeOdtWriterTransferGraph(candidate: unknown): SwDoc {
  if (
    !isRecord(candidate) ||
    candidate.odtTransferModelVersion !== 1 ||
    !Array.isArray(candidate.numRules) ||
    !Array.isArray(candidate.textFormatCollections) ||
    !Array.isArray(candidate.textNodes)
  )
    throw new Error("ODT worker graph schema is unsupported.");
  const record = candidate as unknown as OdtWriterTransferGraph;
...
export interface OdtWriterTransferRecord {
  readonly graph: OdtWriterTransferGraph;
  readonly transferVersion: 3;
}
...
export function createOdtWriterTransfer(document: SwDoc): OdtWriterTransferRecord {
  return { graph: encodeOdtWriterTransferGraph(document), transferVersion: 3 };
}

/** Restores only the current filter-transfer version through canonical Writer operations. @param candidate - Worker value. @returns Canonical graph. */
export function restoreOdtWriterTransfer(candidate: unknown): SwDoc {
  if (
    ...
    candidate.transferVersion !== 3 ||
    !("graph" in candidate)
  )
    throw new Error("ODT worker transfer schema is unsupported.");
  return decodeOdtWriterTransferGraph(candidate.graph);
}
